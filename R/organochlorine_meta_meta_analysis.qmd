...
forest_plot_health <- ggplot(plot_data_health, aes(y = reorder(health_outcome_group, OR), x = OR)) +
  # PI: thin dashed (wider range for future studies)
  geom_errorbarh(aes(xmin = OR_pi_lower, xmax = OR_pi_upper),
                 height = 0.3, linewidth = 0.8, linetype = "dotted", 
                 color = "blue", alpha = 0.6) +
  # CI: solid (summary effect precision)
  geom_errorbarh(aes(xmin = OR_ci_lower, xmax = OR_ci_upper),
                 height = 0.2, linewidth = 1.2, alpha = ifelse(pval > 0.05)) +
  geom_point(aes(size = k), 
             fill = ifelse(pval > 0.05, "grey60", "black"),
             color = "white", stroke = 0.8, shape = 21, alpha = 0.9) +
  scale_size(range = c(4, 12)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red", linewidth = 1) +
  # Labels with k/n + sig indicator
  geom_text(aes(label = paste0("k=", k, "(n=", n, ")", ifelse(pval < 0.05)),
                x = OR * 1.02),  # Position near point
            hjust = 0, size = 4, fontface = "bold") +
  labs(x = "Odds Ratio (95% CI ··· 95% PI)",  # Legend in title
       y = "Adverse Health Outcome",
       caption = "* p < 0.05 | PI: expected range for new studies") +
  ...
  scale_x_log10() +  # Log scale for OR (standard)
  annotation_logticks(sides = "b")  # Minor ticks
...
for (i in seq_along(ceesat_cols)) {
  ...
  # **DOI-OVERLAP VCV CONSTRUCTION** (replaces impute_covariance_matrix)
...
```{r}
# ── USER SETTINGS -----------------------------------------------------------
effect_col   <- "lnH"        # outcome for Hedges' g meta-analysis
variance_col <- "se"         # matching SE column
y_axis_label <- "I2"
...
for (i in seq_along(study_levels)) {
  ...
  # 2c. **DOI-OVERLAP VCV** (replaces impute_covariance_matrix)
  ma_ids_sub <- sort(unique(data_i$ma_e_id))
  R_sub <- R_full[ma_ids_sub, ma_ids_sub]
  row_map <- match(data_i$ma_e_id, ma_ids_sub)
  R_row_sub <- R_sub[row_map, row_map]
...
# **DOI-OVERLAP VCV** (replaces impute_covariance_matrix)
...
mod_overall_filter.OR <- rma.mv(
  yi     = dat_quant_OR$logOR,     # use column reference
  V      = VCV_OR,
  random = list(~1 | ma_id,
                ~1 | ma_e_id),
  data   = dat_quant_OR,
  ...
  robust(dat_quant_OR$ma_id)   # robust (clustered) SEs
...
VCV_H2_OR <- diag(dat_H2_OR$se^2) %*% R_row_H2_OR %*% diag(dat_H2_OR$se^2)

###############################################################################
# 3.  MULTI-LEVEL META-ANALYSIS  ---------------------------------------------
###############################################################################
mod_overall_H2_OR <- rma.mv(
  yi     = dat_H2_OR$lnH,        # fixed column reference
  V      = VCV_H2_OR,
  random = list(~1 | ma_id,
                ~1 | ma_e_id),
  data   = dat_H2_OR,
  ...
  robust(dat_H2_OR$ma_id)     # clustered (robust) SEs
