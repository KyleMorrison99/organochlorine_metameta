...
# Convert to OR scale + add prediction interval (using available columns)
results_list <- results_list %>%
  mutate(
    ...
    # Approximate SE from Wald CI (conservative)
    se_approx = (logOR - ci.lb) / 1.96,
    
    # Prediction interval using t-distribution (df = k-2 for multilevel)
    OR_pi_lb  = exp(logOR - qt(0.975, df = pmax(3, k-2)) * se_approx),
    OR_pi_ub  = exp(logOR + qt(0.975, df = pmax(3, k-2)) * se_approx),
...
plot_data_health <- results_health %>% 
  ...
  mutate(
    ...
    # Approximate SE from Wald CI
    se_approx    = (logOR - ci.lb) / 1.96,
    
    # Prediction intervals (t-distribution, df = k-2)
    OR_pi_lower  = exp(logOR - qt(0.975, df = pmax(3, k-2)) * se_approx),
    OR_pi_upper  = exp(logOR + qt(0.975, df = pmax(3, k-2)) * se_approx)
  )
...
biomark_model <- rma.mv(yi = logOR,  
                      ...
                      mods = ~ biomarker, 
                      control = list(iter.max = 1000, rel.tol = 1e-8)) %>% 
                      robust(biomark_model, dat_quant$ma_id) 
