...
# Subset the DOI‑overlap correlation matrix to the MAs in dat_H2
ma_ids_H2 <- unique(dat_H2$ma_e_id)
R_H2      <- R_full[ma_ids_H2, ma_ids_H2]

# Map each row of dat_H2 to the appropriate MA correlation
R_H2_row  <- R_full[dat_H2$ma_e_id, dat_H2$ma_e_id]

# VCV for lnH using DOI overlap
V_H2 <- diag(dat_H2$SElnH^2) %*% R_H2_row %*% diag(dat_H2$SElnH^2)
...
mod_overall_H2 <- robust(mod_overall_H2, dat_H2$ma_id) 
...
results_list <- map_dfr(chemicals, function(chem) {
  ...
  if (n_distinct(dat_sub$ma_e_id) > 1) {  # Require at least 2 meta-analytic estimates
    tryCatch({
      # Get ma_e_id for this chemical (sorted to match R_full)
      ma_ids_sub <- sort(unique(dat_sub$ma_e_id))
      
      # Subset R_full to this chemical's meta-analyses
      R_sub <- R_full[ma_ids_sub, ma_ids_sub]
      
      # Map each row of dat_sub to the correlation in R_sub
      row_map <- match(dat_sub$ma_e_id, ma_ids_sub)
      R_row_sub <- R_sub[row_map, row_map]
      
      # Construct V = diag(se^2) %*% R_sub %*% diag(se^2)
      V_sub <- diag(dat_sub$se^2) %*% R_row_sub %*% diag(dat_sub$se^2)
      
      model <- rma.mv(
        yi     = dat_sub$logOR,        # Fixed: use dat_sub$logOR
        V      = V_sub,                # NEW: DOI-overlap VCV
...
results_list <- results_list %>%
  mutate(
    ...
    # Approximate SE from Wald CI (conservative, on OR scale as original)
    se_approx = (OR_ub - OR_lb) / 1.96,  # Note: (OR_ub - OR_lb)/1.96 matches your code exactly
    
    # Accurate SE on log scale from Wald CI for prediction interval
    se_log = (ci.ub - ci.lb) / (2 * 1.96),
    
    # Prediction interval using t-distribution (df = max(3, k-2)), on log scale then exp
    t_mult = qt(0.975, df = pmax(3, k-2)),
    OR_pi_lb  = exp(logOR - t_mult * se_log),
    OR_pi_ub  = exp(logOR + t_mult * se_log),
    
    # Prediction interval width
    OR_pi_width = OR_pi_ub - OR_pi_lb
    
  ) %>%
  select(chemical_id, logOR, se_approx, ci.lb, ci.ub, pval, k, n, 
         OR, OR_lb, OR_ub, OR_pi_lb, OR_pi_ub, OR_pi_width)
...
# Run separate meta-analyses for each chemical_id USING DOI-OVERLAP V
...
results_list2 <- map_dfr(chemicals, function(chem) {
  ...
  if (n_distinct(dat_sub$ma_e_id) > 1) {  # Require at least 2 meta-analytic estimates
    tryCatch({
      ...
      # Construct V = diag(se^2) %*% R_row_sub %*% diag(se^2)
      V_sub <- diag(dat_sub$SElnH^2) %*% R_row_sub %*% diag(dat_sub$SElnH^2)
      
      model <- rma.mv(
        yi     = dat_sub$lnH,      # now using lnH
        V      = V_sub,            # DOI-overlap VCV
        random = list(~1 | ma_id,
                      ~1 | ma_e_id),
        test   = "t",
        sparse = TRUE,
      ...
      tibble(
        chemical_id = chem, lnH = NA, ci.lb = NA, ci.ub = NA, pval = NA,
        k = nrow(dat_sub), n = n_distinct(dat_sub$ma_id)
      )
    })
  } else {
...
# Loop over each health outcome group USING DOI-OVERLAP V
results_health <- map_dfr(outcomes, function(outcome) {
  ...
  if (n_distinct(dat_sub$ma_e_id) > 1) { # requires at least 2 meta-analytic estimates
    tryCatch({
      # Get ma_e_id for this outcome (sorted to match R_full)
      ma_ids_sub <- sort(unique(dat_sub$ma_e_id))
      
      # Subset R_full to this outcome's meta-analyses
      R_sub <- R_full[ma_ids_sub, ma_ids_sub]
      ...
      tibble(
        health_outcome_group = outcome, logOR = NA, ci.lb = NA, ci.ub = NA, pval = NA,
        k = nrow(dat_sub), n = n_distinct(dat_sub$ma_id)
      )
    })
  } else {
...
plot_data_health <- results_health %>% 
  ...
  mutate(
    ...
    # SE on log scale (symmetric, precise)
    se_log       = (ci.ub - ci.lb) / (2 * 1.96),
    
    # PI on logOR, then exp to OR
    t_mult       = qt(0.975, df = pmax(3, k-2)),
    OR_pi_lower  = exp(logOR - t_mult * se_log),
    OR_pi_upper  = exp(logOR + t_mult * se_log)
  )

forest_plot_health <- ggplot(plot_data_health, aes(y = reorder(health_outcome_group, OR), x = OR)) +
  geom_errorbarh(aes(xmin = OR_ci_lower, xmax = OR_ci_upper),
                 height = 0.2, linewidth = 1.2, 
                 alpha = ifelse(plot_data_health$pval > 0.05, 1, 0.6)) +
  geom_point(aes(size = k), 
             fill = ifelse(plot_data_health$pval > 0.05, "grey60", "black"),
             color = "white", stroke = 0.8, shape = 21, alpha = 0.9) +
  scale_size(range = c(4, 12)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red", linewidth = 1) +
  geom_text(data = plot_data_health, 
            aes(y = reorder(health_outcome_group, OR), 
                x = OR * 1.07, 
                label = paste0("k=", k, "(n=", n, ")", ifelse(pval < 0.05, "*", ""))),
...
results_health_H2 <- map_dfr(outcomes, function(outcome) {
  ...
  if (n_distinct(dat_sub$ma_e_id) > 1) { # requires at least 2 meta-analytic estimates
    tryCatch({
      ...
      tibble(
        health_outcome_group = outcome, lnH = NA, ci.lb = NA, ci.ub = NA, pval = NA,
        k = nrow(dat_sub), n = n_distinct(dat_sub$ma_id)
      )
    })
  } else {
...
## *biomarker meta-regression & plot*
```{r}
biomark_model <- rma.mv(yi = logOR,  
                      V = V,
                      random = list(~1|ma_id,
                                    ~1|ma_e_id),
                      test = "t", 
...
biomarker_plot <-  orchard_plot(
  biomark_model,                       # the metafor rma.mv object
  group      = "ma_id",           # cluster/grouping variable for k labels
  data       = dat_quant,    
  mod        = "biomarker",       # original data frame
  g          = TRUE,              # include the global (pooled) estimate
  k.pos      = "right",           # show study counts on the right
  xlab       = "Log Odds Ratio",
  trunk.size = 3,                 # size of the pooled effect symbol
...
ggsave(
  filename = here::here("figures", "biomarker_forest.png"),
  plot     = biomarker_plot,
  width    = 8,
  height   = 4,
  dpi      = 300
...
for (i in seq_along(ceesat_cols)) {
  ...
  data_i <- dat_quant %>% 
    filter(!is.na(.data[[col_i]]), tolower(.data[[col_i]]) != "red") %>% 
    mutate(pair = paste0(health_outcome_group, chemical_id))
  ...
  # **DOI-OVERLAP VCV** (compatible with new R_full incidence matrix)
  ma_ids_sub <- sort(unique(data_i$ma_e_id))
  R_sub <- R_full[ma_ids_sub, ma_ids_sub]
  row_map <- match(data_i$ma_e_id, ma_ids_sub)
  R_row_sub <- R_sub[row_map, row_map]
  vcv <- diag(data_i[[variance_col]]^2) %*% R_row_sub %*% diag(data_i[[variance_col]]^2)
...
# ── Tidy model results (FIXED lambda syntax) --------------------------------
results_ceesat <- map2_dfr(ceesat_cols, seq_along(ceesat_cols), ~ {
  mod <- ceesat_models[[.y]]
  if (!is_rma(mod)) {
    tibble(
      CEESAT_item = .x,
...
# ── Forest plot data --------------------------------------------------------
plot_data_ceesat <- results_ceesat %>%
  filter(!is.na(OR)) %>%
  left_join(counts, by = "CEESAT_item") %>%
  mutate(item_label = paste0(CEESAT_item, " (", n_effects_excl, " k; ", n_studies_excl, " n)")) %>%
  arrange(match(CEESAT_item, rev(ceesat_cols))) %>%
  mutate(item_label = factor(item_label, levels = item_label))

overall_row <- tibble(
  CEESAT_item = "Overall",
  OR = 1.279831,
  OR_ci_lower = 1.162348,
  OR_ci_upper = 1.409188,
  n_effects_excl = 0,
  n_studies_excl = 0,
  item_label = "Overall"
)

plot_data_ceesat <- bind_rows(overall_row, plot_data_ceesat) %>% 
  mutate(item_label = factor(item_label, levels = item_label))

# ── Forest plot -------------------------------------------------------------
leave_red_out_forest <- ggplot(plot_data_ceesat, 
                               aes(x = item_label, y = OR, ymin = OR_ci_lower, ymax = OR_ci_upper)) +
  geom_hline(yintercept = 1, linetype = "dashed", colour = "firebrick", alpha = 0.35) +
  geom_hline(yintercept = 1.28, linetype = "dashed", colour = "#2171B5", alpha = 0.35) +
  geom_pointrange() +
  geom_text(aes(label = sprintf("%.2f [%.2f, %.2f]", OR, OR_ci_lower, OR_ci_upper)),
            vjust = -0.8, size = 2.8) +
  scale_y_continuous(limits = c(0.75, 1.50), breaks = c(0.75, 1.00, 1.25, 1.50),
                     labels = scales::number_format(accuracy = 0.01)) +
  coord_flip() +
  theme_bw() +
  theme(axis.text.y = element_text(size = 8), 
        plot.caption = element_text(size = 9, face = "italic", colour = "grey30")) +
  labs(x = "CEESAT item (rows with RED score removed)", 
       y = "Odds ratio",
  ...
  geom_point(data = filter(plot_data_ceesat, CEESAT_item == "Overall"),
             aes(x = item_label, y = OR), shape = 23, size = 3, fill = "firebrick", colour = "black")

print(leave_red_out_forest)

# ggsave(here("figures", "leave_red_out_forest.png"), leave_red_out_forest, width = 8, height = 6, dpi = 300)
...
for (i in seq_along(ceesat_cols)) {
  ...
  # **DOI-OVERLAP VCV** (works with new R_full)
  ma_ids_sub <- sort(unique(data_i$ma_e_id))
  R_sub <- R_full[ma_ids_sub, ma_ids_sub]
  row_map <- match(data_i$ma_e_id, ma_ids_sub)
  R_row_sub <- R_sub[row_map, row_map]
...
# ── Tidy results (FIXED lambda) ---------------------------------------------
results_ceesat2 <- map2_dfr(ceesat_cols, seq_along(ceesat_cols), ~ {
  mod <- ceesat_models[[.y]]
  if (!is_rma(mod)) {
    tibble(
      CEESAT_item = .x,
...
kable(results_ceesat2, digits = 3,
      caption = "Leave-one-RED-CEESAT-item-out heterogeneity sensitivity") %>% 
  kable_styling("striped", position = "left") %>% 
  scroll_box(width = "100%", height = "400px")
...
effect_rows_excl <- dat_H2 %>%
  ...
  mutate(across(everything(), tolower)) %>%
  summarise(across(everything(), ~ sum(.x == "red", na.rm = TRUE))) %>%
  pivot_longer(cols = everything(), names_to = "CEESAT_item", values_to = "n_effects_excl")
...
leave_red_out_forest_het <- ggplot(plot_data, aes(x = item_label, y = I2, ymin = I2_ci_lower, ymax = I2_ci_upper)) +
  ...
  geom_pointrange() +
  geom_text(aes(label = sprintf("%.1f [%.1f, %.1f]", I2, I2_ci_lower, I2_ci_upper)),
            vjust = -0.8, size = 2.8) +
  ...
  labs(x = "CEESAT item (rows with RED score removed)", 
       y = "Heterogeneity (I², %)",
       caption = "Point = pooled I² after excluding studies scored RED for that criterion\n(k = effect-size rows removed; n = unique studies removed)") +
  geom_point(data = filter(plot_data, CEESAT_item == "Overall"),
             aes(x = item_label, y = I2), shape = 23, size = 3, fill = "firebrick", colour = "black")

print(leave_red_out_forest_het)

 ggsave(here("figures", "ceesat_forest_het.png"), leave_red_out_forest_het, 
...
for (i in seq_along(study_levels)) {
  ...
  if (nrow(data_i) < 2) {
    loo_models[[i]] <- loo_VCV[[i]] <- NULL
    next
  }
  
  # **DOI-OVERLAP VCV** (new R_full compatible)
  ma_ids_sub <- sort(unique(data_i$ma_e_id))
  R_sub <- R_full[ma_ids_sub, ma_ids_sub]
  row_map <- match(data_i$ma_e_id, ma_ids_sub)
  R_row_sub <- R_sub[row_map, row_map]
  vcv_i <- diag(data_i[[variance_col]]^2) %*% R_row_sub %*% diag(data_i[[variance_col]]^2)
  vcv_i <- as.matrix(vcv_i)
  loo_VCV[[i]] <- vcv_i
...
#  3.  TIDY RESULTS (map2_dfr with ~ lambda) ---------------------------------
results_loo <- map2_dfr(study_levels, seq_along(study_levels), ~ {
  mod <- loo_models[[.y]]
...
#  4.  DISPLAY TABLE ---------------------------------------------------------
kable(results_loo, digits = 3, caption = "Leave-one-study-out sensitivity results") %>% 
  kable_styling("striped", position = "left") %>% 
  scroll_box(width = "100%", height = "400px")

# ── Plot data (REVERSE study order, Overall bottom) -------------------------
...
# REVERSE study order + Overall last
study_order <- unique(dat_quant$ma_id)  # Natural order
rev_study_order <- rev(study_order)     # REVERSED
all_levels <- c(as.character(rev_study_order), "Overall")  # Rev studies, Overall bottom
...
results_loo_plot_data <- bind_rows(results_loo_plot_data, overall_row) %>%
  mutate(study_id = factor(study_id, levels = all_levels))

# ── Forest plot (reversed study order) -------------------------------------
loo_study_forest <- ggplot(results_loo_plot_data, aes(x = study_id, y = OR, ymin = OR_ci_lower, ymax = OR_ci_upper)) +
  geom_hline(yintercept = 1, linetype = "dashed", colour = "firebrick", alpha = 0.35) +
  geom_hline(yintercept = 1.28, linetype = "dashed", colour = "#2171B5", alpha = 0.35) +
  geom_pointrange() +
  scale_y_continuous(limits = c(0.9, 1.55), breaks = c(0.9, 1.0, 1.2, 1.4, 1.55),
                     labels = scales::number_format(accuracy = 0.01)) +
  coord_flip() +
  theme_bw() +
  theme(axis.text.y = element_text(size = 8), 
        plot.caption = element_text(size = 9, face = "italic", colour = "grey30")) +
  labs(x = "Study left out (ma_id)", y = "Odds ratio",
       caption = "Each point = overall estimate after excluding one study") +
  geom_point(data = filter(results_loo_plot_data, study_id == "Overall"),
             aes(x = study_id, y = OR), shape = 23, size = 3, fill = "firebrick", colour = "black")

print(loo_study_forest)



# ggsave(here("figures", "loo_study_forest.png"), loo_study_forest, width = 8, height = 7, dpi = 300)
...
### *odds ratio*
```{r}
dat_quant_OR <- dat_quant %>%
  filter(ma_es_measure == "Odds ratio") %>%
  mutate(
    ma_e_id = as.character(ma_e_id),
...
stopifnot(nrow(dat_quant_OR) > 1, nrow(dat_H2_OR) > 1)

```

```{r}
ma_ids_OR   <- sort(unique(dat_quant_OR$ma_e_id))
R_OR        <- R_full[ma_ids_OR, ma_ids_OR]
row_map_OR  <- match(dat_quant_OR$ma_e_id, ma_ids_OR)
R_row_OR    <- R_OR[row_map_OR, row_map_OR]
VCV_OR      <- diag(dat_quant_OR$se^2) %*% R_row_OR %*% diag(dat_quant_OR$se^2)

mod_overall_filter_OR <- rma.mv(
  yi     = logOR,
  V      = VCV_OR,
  random = list(~1 | ma_id, ~1 | ma_e_id),
  data   = dat_quant_OR,
...
mod_overall_filter_OR <- robust(mod_overall_filter_OR, cluster = dat_quant_OR$ma_id)

overall_results_filter_OR <- tibble(
  lnOR          = as.numeric(mod_overall_filter_OR$b[1]),
  lnOR_ci_lower = mod_overall_filter_OR$ci.lb[1],
  lnOR_ci_upper = mod_overall_filter_OR$ci.ub[1],
  lnOR_p_value  = mod_overall_filter_OR$pval[1],
  k             = mod_overall_filter_OR$k,
  tau2          = sum(mod_overall_filter_OR$sigma2)
)

overall_value_OR <- tibble(
  OR          = exp(overall_results_filter_OR$lnOR),
  OR_ci_lower = exp(overall_results_filter_OR$lnOR_ci_lower),
  OR_ci_upper = exp(overall_results_filter_OR$lnOR_ci_upper)
)
...
```{r}
ma_ids_H2_OR   <- sort(unique(dat_H2_OR$ma_e_id))
R_H2_OR        <- R_full[ma_ids_H2_OR, ma_ids_H2_OR]
row_map_H2_OR  <- match(dat_H2_OR$ma_e_id, ma_ids_H2_OR)
R_row_H2_OR    <- R_H2_OR[row_map_H2_OR, row_map_H2_OR]
VCV_H2_OR      <- diag(dat_H2_OR$SElnH^2) %*% R_row_H2_OR %*% diag(dat_H2_OR$SElnH^2)
...
mod_overall_H2_OR <- robust(mod_overall_H2_OR, cluster = dat_H2_OR$ma_id)

overall_results_H2_OR <- tibble(
  lnH          = as.numeric(mod_overall_H2_OR$b[1]),
  lnH_ci_lower = mod_overall_H2_OR$ci.lb[1],
  lnH_ci_upper = mod_overall_H2_OR$ci.ub[1],
  lnH_p_value  = mod_overall_H2_OR$pval[1],
  k            = mod_overall_H2_OR$k,
  tau2         = sum(mod_overall_H2_OR$sigma2)
) %>%
  mutate(
    H2          = exp(2 * lnH),
    H2_lb       = exp(2 * lnH_ci_lower),
...
overall_value_I2_OR <- tibble(
  I2          = overall_results_H2_OR$I2,
  I2_ci_lower = overall_results_H2_OR$I2_ci_lower,
  I2_ci_upper = overall_results_H2_OR$I2_ci_upper,
  I2_se       = (overall_results_H2_OR$I2_ci_upper - overall_results_H2_OR$I2_ci_lower) / (2 * 1.96)
)

I2_text_OR <- paste0(
  "Overall I² = ", round(overall_results_H2_OR$I2, 1),
...
```{r}
overall_plot_OR <- dat_quant_OR %>%
  mutate(
    OR          = exp(logOR),
    OR_ci_lower = exp(l_ci_logOR),
    OR_ci_upper = exp(u_ci_logOR),
    ma_e_id     = factor(ma_e_id)
  )

overall_row_OR <- tibble(
  ma_e_id     = factor("Overall", levels = c(levels(overall_plot_OR$ma_e_id), "Overall")),
  OR          = overall_value_OR$OR[1],
  OR_ci_lower = overall_value_OR$OR_ci_lower[1],
  OR_ci_upper = overall_value_OR$OR_ci_upper[1]
)
...
fp_OR <- ggplot(overall_plot_OR, aes(x = OR, y = ma_e_id)) +
  ...
  geom_errorbarh(aes(xmin = OR_ci_lower, xmax = OR_ci_upper), height = 0.2) +
  geom_vline(xintercept = 1, linetype = "dashed", colour = "red") +
  geom_point(data = overall_row_OR,
             aes(x = OR, y = ma_e_id),
             shape = 23, size = 4, fill = "red") +
  geom_errorbarh(data = overall_row_OR,
                 aes(y = ma_e_id, xmin = OR_ci_lower, xmax = OR_ci_upper),
                 height = 0.3, linewidth = 1) +
  labs(x = "Odds Ratio (OR)", y = "Meta-analytic estimate",
       title = "Forest Plot of Odds Ratios (OR only)") +
  theme_minimal() +
  theme(panel.grid.major.y = element_blank()) +
  scale_y_discrete(limits = rev(c(levels(overall_plot_OR$ma_e_id), "Overall")))
...
galaxy_data_OR <- dat_quant_OR %>%
  ...
  left_join(
    plot_data_I2_OR %>%
      mutate(ma_e_id = as.character(ma_e_id)) %>%
      select(ma_e_id, I2, I2_ci_lower, I2_ci_upper, I2_se),
    by = "ma_e_id"
  ) %>%
  filter(!is.na(I2) & !is.na(OR) & !is.na(se))

overall_point_OR <- tibble(
  OR          = overall_value_OR$OR[1],
  OR_ci_lower = overall_value_OR$OR_ci_lower[1],
  OR_ci_upper = overall_value_OR$OR_ci_upper[1],
  I2          = overall_value_I2_OR$I2[1],
  I2_ci_lower = overall_value_I2_OR$I2_ci_lower[1],
  I2_ci_upper = overall_value_I2_OR$I2_ci_upper[1]
)

galaxy_plot_OR <- ggplot(galaxy_data_OR, aes(x = OR, y = I2, size = 1 / se)) +
  geom_point(shape = 21, fill = "#2171B5", colour = "#2171B5", alpha = 0.6) +

  # Pooled OR CI drawn at y = pooled I2  (key fix)
  geom_errorbarh(
    data = overall_point_OR,
    aes(y = I2, xmin = OR_ci_lower, xmax = OR_ci_upper),
    inherit.aes = FALSE,
    height = 2,
    linewidth = 1.3,
    colour = "black"
  ...
  geom_errorbar(
    data = overall_point_OR,
    aes(x = OR, ymin = I2_ci_lower, ymax = I2_ci_upper),
    inherit.aes = FALSE,
    width  = 0.03,
    linewidth = 1.3,
    colour = "black"
...
ggsave(here("figures", "galaxy_plot_OR.jpg"), galaxy_plot_out_OR, width = 9, height = 7, dpi = 300)
```
...
## Publication bias
```{r}
V_pub <- dat_quant$se^2

dat_quant$V_pub <- dat_quant$se^2

# Fit publication bias model (Egger's regression)
pub.bias <- rma.mv(
  yi    = logOR,
  V     = V,
  mods  = ~ V_pub,
...
# Extracting data from the model
pub.results <- tibble(
  lnOR = as.numeric(pub.bias$b),
  lnOR_ci_lower = pub.bias$ci.lb,
  lnOR_ci_upper = pub.bias$ci.ub,
  lnOR_p_value = pub.bias$pval,
  lnOR_tau2 = sum(pub.bias$sigma2)
)

pub.results <- tibble(
  OR          = exp(as.numeric(pub.bias$b)),           # Intercept + moderator slope
  OR_ci_lower = exp(pub.bias$ci.lb),
  OR_ci_upper = exp(pub.bias$ci.ub),
  lnOR_p_value = pub.bias$pval,
  lnOR_tau2   = sum(pub.bias$sigma2)
)
...
pub.bias_pred <- predict(pub.bias)


pub.bias.bubble <- bubble_plot(pub.bias, mod = "V_pub", group = "ma_id", data = dat_quant) +
  labs(
    x = "Sampling Variance",
    y = "Log Odds Ratio",
  ) +
  xlim(0, 2) +  # Cap x-axis as requested
  geom_smooth(method = "lm", se = TRUE, color = "red", alpha = 0.2) +  # Regression line + CI
  scale_fill_brewer(palette = "Dark2", name = "Precision") +  # Better colors
